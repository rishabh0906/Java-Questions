...
public class StringSet {
    ...
    public static int isMatch_02(String s, String p, int n, int m, int[][] dp) {
        if (n == 0 && m == 0)
            return dp[n][m] = 1;
        if (m == 0)
            return dp[n][m] = 0;
        ...
        if (ch1 != '$' && (ch1 == ch2 || ch2 == '.'))
            return dp[n][m] = isMatch_02(s, p, n - 1, m - 1, dp);
        else if (ch2 == '*') {
            boolean res = false;

            if (n > 0 && m > 1 && (p.charAt(m - 2) == '.' || p.charAt(m - 2) == ch1))
                res = res || isMatch_02(s, p, n - 1, m, dp) == 1;
            res = res || isMatch_02(s, p, n, m - 2, dp) == 1;
    ...
    public static boolean isMatch_02(String s, String p) {
        ...
        return isMatch_02(s, p, n, m, dp) == 1;
